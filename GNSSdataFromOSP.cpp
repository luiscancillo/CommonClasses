...
bool GNSSdataFromOSP::extractGLOEphemeris(int ch, unsigned int &sv, double &tTag, int (&bom)[8][4]) {
	...
	tTag = getInstantGPSdate(1996 + (n4-1)*4, 1, nt, 0, 0, (float) tb) - 3*60*60;
	bom[0][0] = (int) tTag;													//Toc
	...
	bom[0][3] = ((int) getTowGNSSinstant(tTag) + 518400) % 604800;		//seconds from UTC week start (mon 00:00). Note that GPS week starts sun 00:00.
